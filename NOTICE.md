...
EMON — производная работа от **[NIDAS](https://github.com/S4mpsa/NIDAS)** (Networked
...
поэтому EMON как производная работа обязан распространяться на тех же условиях. Полный текст
...
| `lib/graphics/graphics.lua` | На основе `lib/graphics/graphics.lua` из NIDAS (рендеринг полублочными символами). Переписан: исправлена обработка нечётных границ прямоугольника, убрана система буферов видеопамяти (снимает требование GPU третьего уровня). |
...
EMON сопоставляет по лейблам. См. раздел «Чем отличается от NIDAS» в [README.md](README.md).
